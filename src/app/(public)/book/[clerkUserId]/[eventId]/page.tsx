...
export default async function BookEventPage({
  ...
  const calendarUserData = {
    id: (await calendarUser)?.id,
    fullName: `${(await calendarUser)?.firstName || ""} ${
      (await calendarUser).lastName || ""
    }`.trim(),
  };
  ...
  if (validTimes.length === 0) {
    return <NoTimeSlots event={event} calendarUser={calendarUserData} />;
  }
  ...
  return (
    <Card className="max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>
          Book {event.name} with {(await calendarUser).fullName}
        </CardTitle>
        {event.description && (
